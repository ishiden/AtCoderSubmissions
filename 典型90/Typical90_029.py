...
# https://github.com/tsutaj/pastbook-2-source-code/blob/main/python/chapter6/lazy-segment-tree.py
    def __init__(
        self,
        n,  # 列の長さ
...
def main():
  ...
  lazy_seg = LazySegmentTree(W, 0, -1, max, rewrite, rewrite)
  lazy_seg.build([0]*W)

  for _ in range(N):
    L, R = map(int, input().split())
    ans = lazy_seg.fold(L-1, R) + 1
    print(ans)

    lazy_seg.update(L-1, R, ans)
